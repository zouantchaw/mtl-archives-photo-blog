...
export type FieldSetType = "text" | "email" | "password" | "checkbox";
...
export const convertFormDataToPhotoDbInsert = (
  ...
  const photoForm =
    formData instanceof FormData
      ? (Object.fromEntries(formData) as Partial<PhotoFormData>)
      : formData;
  ...
  const result: PhotoDbInsert = {
    ...(photoForm as Partial<PhotoFormData> & {
      filmSimulation?: FilmSimulation;
    }),
    id: generateId || !photoForm.id ? generateNanoid() : photoForm.id,
    url: photoForm.url || "",
    extension: photoForm.extension || "",
    aspectRatio: photoForm.aspectRatio
      ? toFixedNumber(parseFloat(photoForm.aspectRatio), 6)
    ...
    takenAt: photoForm.takenAt || now.toISOString(),
    takenAtNaive: photoForm.takenAtNaive
      ? format(parseISO(photoForm.takenAtNaive), "yyyy-MM-dd HH:mm:ss")
      : formattedNow,
  };
